...
* 产品：定义了产品的接口
* 具体产品：实现了产品接口。
* 创建者/工厂：声明了“工厂方法”，该方法返回一个产品类型的对象。创建者通常包含一些依赖于产品对象的核心业务逻辑。
* 具体创建者/具体工厂：重写基类的工厂方法，返回一个具体产品的实例。

#### 理解

工厂方法：「工厂」表示是一个创建对象的地方,比如抽象类HiringManager。「方法」表示通过方法来创建对象,比如方法makeInterviewer()。

大白话：有一个工厂,即抽象类。它有一个方法，这个方法可以用来创建不同类型的对象,即抽象类中的抽象方法。

工厂是抽象工厂，是抽象基类；工厂里面有抽象方法，方法是工厂方法，它是用来创建产品的方法；
...
我们用一个经典的例子——日志记录器——来演示。我们需要一个记录日志的工具，它可能将日志输出到本地文件，也可能输出到控制台，未来还可能输出到网络。
...
#### 工厂方法和抽象工厂的区别
工厂方法： 关注单一产品的创建，它通过子类化来决定具体实例化哪一个产品。
...
“表示”指的是最终构建出来的那个复杂对象的最终形态、外观、结构或具体内容。
