...
* 产品：定义了产品的接口
* 具体产品：实现了产品接口。
* 抽象工厂：声明了“工厂方法”，该方法返回一个产品类型的对象。创建者通常包含一些依赖于产品对象的核心业务逻辑。
* 具体工厂：重写基类的工厂方法，返回一个具体产品的实例。

#### 理解

工厂方法：

「工厂」表示是一个创建对象的地方,比如抽象类HiringManager。

「方法」表示通过方法来创建对象,比如抽象类中的抽象方法makeInterviewer()。

描述：有一个工厂,即抽象类。它有一个方法，这个方法可以用来创建不同类型的对象,即抽象类中的抽象方法。

#### 示例
针对demo_1:

interviewer包中，是产品类，他们都有一个动作：askQuestions。意思就是每个产品都可以问问题，但是askQuestions内部是它们各自的逻辑。

核心的类是：HiringManager,它是抽象类。抽象类里面有抽象方法,体现了"延迟创建"。

核心思想：定义一个用于创建对象的接口「HiringManager中的makeInterviewer」，但让子类决定实例化哪一个类。工厂方法让类的实例化延迟到子类。

针对demo_4

日志记录器

我们需要一个记录日志的工具，它可能将日志输出到本地文件，也可能输出到控制台。

我们创建一个“工厂”的抽象基类。它声明了创建Logger的工厂方法，但同时它可能包含一些不依赖于具体Logger类型的核心业务逻辑。

优势

解耦：客户端代码（Client）只依赖于抽象的 LoggerFactory 和 Logger，完全不知道 FileLogger 或 ConsoleLogger 的存在。这使得增加新的日志类型（比如 NetworkLogger）时，客户端代码完全不需要修改

符合开闭原则：

* 对扩展开放： 要增加一种新的产品（如 DatabaseLogger），你只需要新增一个 DatabaseLogger 类和一个 DatabaseLoggerFactory 类。

* 对修改封闭： 现有的 LoggerFactory、FileLogger、ConsoleLogger 以及客户端代码都不需要做任何修改。

将产品创建的逻辑集中化：创建对象的复杂逻辑（比如需要配置参数、依赖其他对象等）被封装在具体的工厂类中（如 FileLoggerFactory.createLogger() 里），避免了在代码中散落着大量的 new 关键字，使代码更清晰、更易于维护。

### 抽象工厂

#### 工厂方法和抽象工厂的区别
简单工厂：如果在多个地方要创建一个对象，这个对象属于同一个类，且创建时有比较复杂的逻辑而非简单的赋值，就用简单工厂。只有一个具体的工厂类，通过传入的参数（如一个字符串 "file"）在 if/else 或 switch 语句中决定创建哪个产品。

工厂方法： 关注单一产品的创建，它通过子类化来决定具体实例化哪一个产品。如果在多个地方要创建一个对象，这个对象属于不同的类，那么就用工厂方法。每新增一种产品，就新增一种工厂方法的实现。它是一个抽象的工厂类和多个具体的工厂子类。每个子类负责创建一种产品。

抽象工厂： 关注产品家族的创建，它提供一个接口来创建一系列相关或依赖的对象，而无需指定它们的具体类。

建造者：分步构建，逐步设置参数，适用于创建复杂、需要高度定制的产品，客户可以精确控制对象的各个组成部分，自定义套餐。

### 建造者
核心思想：“我想要一个按我自己的规格一步步组装的东西”。它将一个复杂对象的构建与它的表示（最终构建出来的那个复杂对象的最终形态、外观、结构或具体内容）分离，使得同样的构建过程可以创建不同的表现形式。

解释：建造者模式将复杂对象的构建过程分解成一系列清晰的步骤，使得代码不仅易于编写，更易于阅读和维护。

避免了“构造函数污染”（这是建造者模式解决的关键问题）
...
### 原型模式
很多时候我们不需要从零开始创造，而是在现有成果的基础上进行适当的修改和定制。
...
创建现有对象的副本不是最终目标，而是一个起点。你可以再副本基础上进行定制化修改，得到你想要的具体对象。而不是经历从头创建对象并进行设置的麻烦。
...
无论是饿汉、懒汉，单例的唯一性是以单个Java虚拟机为边界的。所有单例在分布式环境中不是唯一的。


### 桥接模式
核心思想：将网页抽象（WebPage）与主题实现（Theme）分离，让它们可以独立变化。

核心思想：不要建立一个固定的、庞杂的继承体系，而是识别出系统中哪些是抽象维度，哪些是实现维度，然后用“搭桥”（组合）的方式将它们连接起来，从而获得更大的灵活性。
...
不需要修改原有类的代码，通过包装（装饰）来增强功能，符合"开闭原则" - 对扩展开放，对修改关闭，替代继承的灵活方案，避免类爆炸，灵活组合功能
````
// 继承方式（不灵活）
class BasicCoffee {}
...
1. 简化复杂系统，提供更简单、更易用的高层接口，降低客户端与子系统的耦合度
2. 统一入口，充当多个子系统组件的"门面"，客户端只需要与外观类交互。
...
优点：大幅减少内存占用，减少对象创建数量，提高性能

缺点：增加系统复杂性，需要分离内部和外部状态，可能引入线程安全问题
...
````
### 迭代器模式
核心概念：迭代器模式提供一种方法顺序访问一个聚合对象中的各个元素，而又不暴露该对象的内部表示。
...
### 中介者模式
比较难理解
![ChatGPT Image 2025年11月4日 18_03_20.png](src/main/resources/ChatGPT Image 2025年11月4日 18_03_20.png)
...
见代码demo_2
````
#### 状态模式（State Pattern）的关键概念

| 概念 | 在例子中 |
| :--- | :--- |
| **Context (上下文)** | `Brush` —— 🎨 持有当前状态对象，并把请求委托给它 |
| **State (状态接口)** | `ColorState` —— 定义所有状态共同行为 |
| **ConcreteState (具体状态)** | `RedState / BlueState / GreenState` —— 实现不同的行为 |

#### 核心思想
对象（画笔）在内部状态（颜色）改变时，行为（绘制输出）也改变。
...
通过封装状态相关的行为，让对象（Brush）能够根据当前状态（private ColorState currentColor;）动态改变行为（public void draw(int x, int y) {}），而不需要在代码中写满复杂的条件判断语句。
...
## 比较：
三个工厂：简单工厂模式只有一个工厂，根据参数使用if/else或switch判断创建哪一种对象。它创建对象不是简单的赋值操作，而是有一定的逻辑的。但是它产品和工厂不解耦。
工厂方法模式有一个抽象基类，抽象基类中有一个抽象方法用来创建具体对象，但是这个创建对象的方法都是延迟的具体工厂的方法中。同时抽象基类中包含另一个非抽象的方法，该方法包含不依赖具体产品的核心业务逻辑。它的具体工厂和具体产品是解耦的。同时符合开闭原则，新增工厂和产品时，客户端源代码不需修改，只需新增新工厂和新产品的内容。
抽象工厂模式，有一个工厂接口，该接口中定义多个创建不同类型产品的方法。然后可以有多个实现类，实现类中各自创建自己相关的产品。
建造者模式：有一个引导者builder,你能通过这个引导者将复杂对象的创建分成若干个步骤，最后调用builder.build()得到你想要的产品，同时能避免构造方法污染。
...
只有一个具体的工厂类，通过传入的参数（如一个字符串 "file"）在 if/else 或 switch 语句中决定创建哪个产品。

工厂方法：

如果在多个地方要创建一个对象，这个对象属于不同的类，那么就用工厂方法。每新增一种产品，就新增一种工厂方法的实现。

它是一个抽象的工厂类和多个具体的工厂子类。每个子类负责创建一种产品。